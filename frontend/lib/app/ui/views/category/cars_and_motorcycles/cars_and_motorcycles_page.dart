...
class CarsAndMotorcyclesPage extends StatefulWidget {
  const CarsAndMotorcyclesPage({Key? key}) : super(key: key);

  @override
  _CarsAndMotorcyclesPageState createState() => _CarsAndMotorcyclesPageState();
}

class _CarsAndMotorcyclesPageState extends State<CarsAndMotorcyclesPage> {
  ...
  String? _currentUserToken; // Per il token utente
  ...
  Future<void> _loadAuctions() async {
    ...
    if (_currentUserToken != null) {
      try {
        await auctionProvider.fetchAuctionsByCategory(_currentUserToken!, '7'); // Passiamo il token e la categoria
        setState(() {
          isLoading = false;
        });
      } catch (e) {
